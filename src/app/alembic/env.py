...
from sqlalchemy import engine_from_config # type: ignore[import-not-found]
from sqlalchemy import pool # type: ignore[import-not-found]

from alembic import context # type: ignore[import-not-found]

from app.models_db import Base  # contains metadata
...
# assert "@" in url, "sqlalchemy.url not set correctly"
...
def run_migrations_online() -> None:
    ...
    """
    url = config.get_main_option("sqlalchemy.url")
    assert url is not None, "sqlalchemy.url not set in alembic.ini"
    connectable = create_engine(url, pool_pre_ping=True)
